...
def verify_image_label(args):
    ...
    im_file, im_file2, lb_file, prefix, prefix2, keypoint, num_cls, nkpt, ndim = args
    # Number (missing, found, empty, corrupt), message, segments, keypoints
    nm, nf, ne, nc, msg, msg2, segments, keypoints = 0, 0, 0, 0, '', '', [], None
    try:
        # Verify images
        ...
        # Verify images
        im2 = Image.open(im_file2)
        im2.verify()  # PIL verify
        shape2 = exif_size(im2)  # image size
        shape2 = (shape2[1], shape2[0])  # hw
        assert (shape2[0] > 9) & (shape2[1] > 9), f'image size {shape2} <10 pixels'
        assert im2.format.lower() in IMG_FORMATS, f'invalid image format {im2.format}'
        if im2.format.lower() in ('jpg', 'jpeg'):
            with open(im_file2, 'rb') as f2:
                f2.seek(-2, 2)
                if f2.read() != b'\xff\xd9':  # corrupt JPEG
                    ImageOps.exif_transpose(Image.open(im_file2)).save(im_file2, 'JPEG', subsampling=0,
                                                                       quality=100)
                    msg2 = f'{prefix2}WARNING ⚠️ {im_file2}: corrupt JPEG restored and saved'
        ...
        if os.path.isfile(lb_file):
            ...
            if nl:
                if keypoint:
                    ...
                    # Clip slightly out-of-bounds coords instead of dropping the image
                    lb[:, 1:] = np.clip(lb[:, 1:], 0, 1)
                ...
                if len(i) < nl:  # duplicate row check
                    ...
                    msg2 = f'{prefix}WARNING ⚠️ {im_file}: {nl - len(i)} duplicate labels removed'
            else:
                ne = 1  # label empty
        ...
        lb = lb[:, :5]
        return im_file, im_file2, lb, shape, segments, keypoints, nm, nf, ne, nc, msg, msg2
    except Exception as e:
        nc = 1
        msg = f'{prefix}WARNING ⚠️ {im_file}: ignoring corrupt image/label: {e}'
        msg2 = f'{prefix2}WARNING ⚠️ {im_file2}: ignoring corrupt image/label: {e}'
        return [None, None, None, None, None, None, nm, nf, ne, nc, msg, msg2]
...
def check_det_dataset(dataset, autodownload=True):
    ...
    # Checks
    for k in 'train', 'train2', 'val', 'val2':
        if k not in data:
            raise SyntaxError(
    ...
    for k in 'train', 'train2', 'train3', 'val', 'val2', 'val3', 'test':
    ...
    # Parse yaml
    train, train2, val, val2, test, s = (data.get(x) for x in ('train', 'train2', 'val', 'val2', 'test', 'download'))
    if val:
    ...
    if val2:
        val2 = [Path(x).resolve() for x in (val2 if isinstance(val2, list) else [val2])]  # val2 path
        if not all(x.exists() for x in val2):
            name2 = clean_url(dataset)  # dataset name with URL auth stripped
            m2 = f"\nDataset '{name2}' images not found ⚠️, missing paths %s" % [str(x) for x in val2 if not x.exists()]
            if s and autodownload:
                LOGGER.warning(m2)
            else:
                m2 += f"\nNote dataset download directory is '{DATASETS_DIR}'. You can update this in '{SETTINGS_YAML}'"
                raise FileNotFoundError(m2)
            t2 = time.time()
            if s.startswith('http') and s.endswith('.zip'):  # URL
                safe_download(url=s, dir=DATASETS_DIR, delete=True)
                r2 = None  # success
                ...
                r2 = exec(s, {'yaml': data})  # return None
            dt2 = f'({round(time.time() - t2, 1)}s)'
            s = f"success ✅ {dt2}, saved to {colorstr('bold', DATASETS_DIR)}" if r2 in (0, None) else f'failure {dt2} ❌'
            LOGGER.info(f'Dataset download {s}\n')

...
class BasePredictor:
    ...
    def preprocess(self, im, im2, im3=None):
        """Prepares input image before inference.

        Args:
            im (torch.Tensor | List(np.ndarray)): RGB, BCHW for tensor, [(HWC) x B] for list.
            im2 (torch.Tensor | List(np.ndarray)): IR, same format.
            im3 (torch.Tensor | List(np.ndarray), optional): Depth, same format.
        """
        not_tensor, not_tensor2 = not isinstance(im, torch.Tensor), not isinstance(im2, torch.Tensor)
        has_im3 = im3 is not None
        not_tensor3 = has_im3 and not isinstance(im3, torch.Tensor)
        if not_tensor or not_tensor2 or not_tensor3:
            p = self.pre_transform(im, im2, im3)
            im = np.stack([item[0] for item in p])
            im2 = np.stack([item[1] for item in p])
            ...
            if has_im3:
                im3 = np.stack([item[2] for item in p])
                im3 = im3[..., ::-1].transpose((0, 3, 1, 2))
        ...
        img2 = img2.half() if self.model.fp16 else img2.float()
        if not_tensor2:
            img2 /= 255
        img3 = None
        if has_im3:
            img3 = im3.to(self.device)
            img3 = img3.half() if self.model.fp16 else img3.float()
            if not_tensor3:
                img3 /= 255
        return (img, img2, img3) if has_im3 else (img, img2)

    def pre_transform(self, im, im2, im3=None):
        """Pre-transform input image before inference.

        Args:
            im (List(np.ndarray)): RGB images.
            im2 (List(np.ndarray)): IR images.
            im3 (List(np.ndarray), optional): Depth images.
        ...
        if im3 is not None:
            return [LetterBox(self.imgsz, auto=auto, stride=self.model.stride)(image=x, image2=y, image3=z)
                    for x, y, z in zip(im, im2, im3)]
        return [LetterBox(self.imgsz, auto=auto, stride=self.model.stride)(image=x, image2=y)
                for x, y in zip(im, im2)]
    ...
    def stream_inference(self, source=None, model=None):
        ...
        has_depth = len(self.dataset) >= 3
        if has_depth:
            # 三个模态图像数必须一致，zip 会静默截断，显式断言防数据错位
            assert len(self.dataset[0]) == len(self.dataset[1]) == len(self.dataset[2]), \
                f"RGB/IR/Depth dataset sizes mismatch: {len(self.dataset[0])}/{len(self.dataset[1])}/{len(self.dataset[2])}"
            batches = zip(self.dataset[0], self.dataset[1], self.dataset[2])
        else:
            batches = zip(self.dataset[0], self.dataset[1])
        for batch in batches:
            self.run_callbacks('on_predict_batch_start')
            if has_depth:
                batch1, batch2, batch3 = batch
                path3, im0s3, vid_cap3, s3 = batch3
            ...
            with profilers[0]:
                if has_depth:
                    im, im2, im3 = self.preprocess(im0s, im0s2, im0s3)
                else:
            ...
            # Inference
            with profilers[1]:
                preds = self.model(im, im2, im3, augment=self.args.augment, visualize=visualize)

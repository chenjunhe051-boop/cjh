...
class AutoBackend(nn.Module):
    ...
    def forward(self, im, im2, im3=None, augment=False, visualize=False):
        """
        Runs inference on the YOLOv8 MultiBackend model.

        Args:
            im (torch.Tensor): The image tensor to perform inference on.
            im2 (torch.Tensor): The image tensor to perform inference on.
            im3 (torch.Tensor, optional): The depth image tensor (3-modal models).
        ...
        if self.pt or self.nn_module:  # PyTorch
            if augment or visualize:
                y = self.model(im, im2, im3, augment=augment, visualize=visualize)
            else:
            ...
            self.net.setInput(im, im2)
            y = self.net.forward()
        elif self.onnx:  # ONNX Runtime
            im = im.cpu().numpy()  # torch to numpy
            im2 = im2.cpu().numpy()  # torch to numpy
            y = self.session.run(self.output_names, {self.session.get_inputs()[0].name: (im, im2)})
        elif self.xml:  # OpenVINO
            im = im.cpu().numpy()  # FP32
            im2 = im2.cpu().numpy()  # FP32
            y = list(self.executable_network([im, im2]).values())

...
# ========================= 阶段2: 全解冻, 低学习率精调 (180 epoch) =========================
print("\n" + "=" * 70)
print("Stage 2: freeze=0, low lr fine-tuning (max 180 epochs)")
print("=" * 70)

# 新建 YOLO 对象，加载阶段1最佳权重 + 预训练 backbone，全新 optimizer/scheduler
model_s2 = YOLO("ultralytics/models/v8/DEYOLO.yaml")
best_path = "my_project/exp_3mod_s1/weights/best.pt"

if not Path(best_path).exists():
    best_path = "my_project/exp_3mod_s1/weights/last.pt"

model_s2.model.load_state_dict(matched_dict, strict=False)
ckpt_s1 = torch.load(best_path, map_location='cuda', weights_only=False)
model_s2.model.load_state_dict(ckpt_s1['model'].state_dict(), strict=False)

print(f"Stage 2 init: loaded {best_path}")
...
model_s2.train(
    ...
    plots=False,
    rect=False,
    freeze=0,                # 全解冻，真正的精调阶段
    # 不设置 resume → 全新 optimizer + cosine lr 从 lr0 重新开始
    ...
    cos_lr=True,
    lr0=0.001,               # 全解冻必须低 lr，防止破坏已收敛 head
    lrf=0.00001,
)